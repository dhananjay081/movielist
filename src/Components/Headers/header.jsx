    ...
    <div className='header fixed top-0 left-0 w-full bg-transparent z-50 shadow-md'>
      <div className='flex items-center justify-between p-4 min-[480px]:gap-8 backdrop-blur-lg'>
        ...
        <div className={`flex-col min-[480px]:flex-row gap-4 min-[480px]:gap-8 min-[480px]:flex ${isOpen ? 'flex' : 'hidden'}`}>
          <Link to="/movies/popular" className='text-white hover:text-gray-300' onClick={closeMenu}>Popular</Link>
          <Link to="/movies/top_rated" className='text-white hover:text-gray-300' onClick={closeMenu}>Top Rated</Link>
          <Link to="/movies/upcoming" className='text-white hover:text-gray-300' onClick={closeMenu}>Upcoming</Link>
        </div>
      </div>
    </div>
  );
